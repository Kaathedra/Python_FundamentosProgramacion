...
def buscar_pelicula(peliculas, titulo_buscado):
    for i in range(len(peliculas)):
        if peliculas[i]["titulo"].lower() == titulo_buscado.strip().lower():
            return i
    return -1
...
def ejecutar_busqueda(peliculas):
    ...
    if posicion != -1:
        print("\n--- Película Encontrada ---")
        mostrar_datos_pelicula(peliculas[posicion])
    else:
...
def eliminar_pelicula(peliculas):
    ...
    if posicion != -1:
        eliminada = peliculas.pop(posicion)
        print(f"✅ La película '{eliminada['titulo']}' fue eliminada con éxito.")
    else:
...
def actualizar_recomendacion(peliculas):
    for pelicula in peliculas:
        if pelicula["calificacion"] >= 4.0:
            pelicula["recomendada"] = True
        else:
            pelicula["recomendada"] = False
